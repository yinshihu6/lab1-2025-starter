...
class Profiler(object):
    ...
    def profile(self) -> dict:
        ...
        pytorch2timeloop.convert_model(
            ...
            model_name=self.sub_dir,
            save_dir=self.top_dir,
            convert_fc=self.convert_fc,
            exception_module_names=self.exception_module_names + ['pool']
        )
        ...
        def get_cmd(layer_id):
            ...
            f"{self.base_dir/self.top_dir/self.sub_dir}/\\[layer{layer_id}\\]*.yaml " \
            f"-o {cwd} > /dev/null 2>&1"
